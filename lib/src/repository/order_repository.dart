...
Future<Stream<Order>> getOrders() async {
  ...
  final String url =
      '${GlobalConfiguration().getString('api_base_url')}orders?${_apiToken}with=user;productOrders;productOrders.product;productOrders.product.market;productOrders.options;orderStatus;payment&search=user.id:${_user.id}&searchFields=user.id:=&orderBy=id&sortedBy=desc';
      print('GETTING USER ORDERS: ' + url);
  try {
    final client = new http.Client();
    final streamedRest = await client.send(http.Request('get', Uri.parse(url)));

    print("get orders URI:");
    print(url.toString());
    ...
    return streamedRest.stream
        ...
        .map((data) {
          print('printing data that is sending further: $data');
...
Future<Stream<Order>> getOrder(orderId) async {
  ...
  final String _apiToken = 'api_token=${_user.apiToken}&';
  final String url =
      '${GlobalConfiguration().getString('api_base_url')}orders/$orderId?${_apiToken}with=user;productOrders;productOrders.product;productOrders.product.market;productOrders.options;orderStatus;deliveryAddress;payment';
  final client = new http.Client();
  final streamedRest = await client.send(http.Request('get', Uri.parse(url)));

  print("To get order details: $url");
...
Future<Order> addOrder(Order order, Payment payment) async {
  ...
  print("url for placing order: " + url);
  final client = new http.Client();
