...
class Helper {
  ...
  static double getScreenHeight(BuildContext widgetContext)
  {
    return MediaQuery.of(widgetContext).size.height.toDouble();
  }

  static double getScreenWidth(BuildContext widgetContext)
  {
    return MediaQuery.of(widgetContext).size.width.toDouble();
  }
...
//        },
  ...
  static Widget getPrice(double myPrice, BuildContext context,
    ...
    static Widget getPrice1(double myPrice, BuildContext context,
      {TextStyle style}) {
    if (style != null) {
      style = style.merge(TextStyle(fontSize: style.fontSize + 2));
    }
  ...
  static double getOrderServiceFee(Order order)
  {
    print("inside getOrderServiceFee");
    print(order.productOrders.isEmpty);
    double total = 0.0;
    double service_fee = 0.0;
    int index = order.productOrders.length > 0 ? order.productOrders.length - 1 : 0;
    total = order.productOrders.isNotEmpty ? getTotalOrderPrice(order.productOrders[index]) : 0.0;
//    service_fee = (order.serviceFee/100) * total;
    service_fee = order.serviceFee;
    service_fee = double.parse((service_fee).toStringAsFixed(2));
//    print(service_fee);
  ...
  static double getTotalOrdersPrice(Order order) {
    ...
    print("service fee ${order.serviceFee}");
    total = total + order.serviceFee;
//    total = total + ((order.serviceFee/100) * total);
  ...
  static bool canDelivery(Market _market, {List<Cart> carts}) {
    ...
    print('DELIVERY RANGEee1: ' + _market.deliveryRange.toString());
    ...
    // print(_market);
    print('DELIVERY RANGEee2: ' + _market.deliveryRange.toString());
    print('DELIVERY RANGE: ' + _market.name);
    print('DELIVERY UNIT: ' + _unit.toString());
    print('DISTANCE: ' + _distance.toString());
    print('DELIVERY RANGE: ' + deliveryAddress.value.isUnknown().toString());
