...
class CategoryController extends ControllerMVC {
  ...
  bool loadCart = false;
  List<Cart> carts = [];
  static int totalCategoryProducts = 0;
  static int totalMarketProducts = 0;
  static bool noProducts = false;

  CategoryController() {
    this.scaffoldKey = new GlobalKey<ScaffoldState>();
  }

  void listenForProductsByCategory(int skip, int take,
      {String id, String message}) async {
    final Stream<Product> stream = await getProductsByCategory(skip, take, id);
  ...
  void listenForCategory({String id, String message}) async {
    ...
    stream.listen((Category _category) {
      ...
      print(a);
      print("error caught for category");
      scaffoldKey?.currentState?.showSnackBar(SnackBar(
        content: Text(S.of(context).verify_your_internet_connection),
      ));
    }, onDone: () {
  ...
  bool isSameMarkets(Product product) {
    if (carts.isNotEmpty) {
      print('inside category controller isSame: ' +
  ...
  Future<void> refreshCategory(String id, {String message}) async {
    products.clear();
    category = new Category();

    listenForProductsByCategory(0, 10,
        id: id, message: S.of(context).category_refreshed_successfuly);
    listenForCategory(
        id: id, message: S.of(context).category_refreshed_successfuly);
  }
}
